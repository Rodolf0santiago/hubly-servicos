...
const iconMap: Record<string, React.ComponentType<any>> = {
  ClipboardList, Search, Award, Activity, TrendingUp, Handshake, LayoutDashboard, Target, ShieldCheck, Clock, Zap, BarChart, Users, Star, Home
};

export const DEFAULT_COMO_FUNCIONA_PAGE = {
  ...
  mission_desc: "Integrar clientes e empresas qualificadas através de um processo transparente, organized e monitorado, garantindo excelência na prestação de serviços e melhoria contínua de toda a rede parceira.",
...
interface ComoFuncionaContentProps {
  data?: any;
}

export default function ComoFuncionaContent({ data }: ComoFuncionaContentProps) {
  const info = { ...DEFAULT_COMO_FUNCIONA_PAGE, ...data };
  ...
  const timelineSteps = Array.isArray(info.timeline_steps) ? info.timeline_steps : DEFAULT_COMO_FUNCIONA_PAGE.timeline_steps;
    ...
    <main className="flex-1 flex flex-col items-center w-full overflow-x-hidden relative bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
      ...
      <section className="w-full pt-40 pb-20 md:pt-56 md:pb-32 px-6 md:px-12 max-w-[1200px] mx-auto text-center">
        ...
        >
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-extrabold tracking-tight mb-8 leading-tight font-montserrat">
            {info.hero_title_part1} <span className="text-brand-emerald">{info.hero_title_part2}</span> {info.hero_title_part3}
          </h1>
      ...
      <section className="w-full py-20 bg-white/50 dark:bg-slate-900/30 backdrop-blur-md border-y border-slate-200 dark:border-slate-800 px-6 md:px-12 relative">
        <div className="max-w-[1440px] mx-auto">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold font-montserrat mb-4 text-brand-navy dark:text-white">{info.timeline_title}</h2>
            <p className="text-slate-500 dark:text-slate-400 max-w-2xl mx-auto">{info.timeline_subtitle}</p>
          </motion.div>
                ...
                return (
                  <motion.div key={i} variants={fadeUp} className="flex flex-col items-center text-center group">
                    <div className="w-24 h-24 rounded-2xl bg-white dark:bg-slate-800 shadow-xl shadow-brand-navy/5 flex items-center justify-center border border-slate-100 dark:border-slate-700 mb-6 relative group-hover:scale-110 group-hover:border-brand-emerald/50 transition-all duration-300">
                      <StepIcon className="w-10 h-10 text-brand-emerald" />
        ...
        <div className="max-w-[1200px] mx-auto flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-brand-emerald/20 text-brand-emerald font-semibold text-sm mb-6">
              ...
              {info.marketing_badge}
            </div>
            <h2 className="text-3xl md:text-5xl font-bold font-montserrat mb-6 leading-tight">
              {info.marketing_title}
            </h2>
            <p className="text-lg text-slate-600 dark:text-slate-300 mb-6 leading-relaxed">
              {info.marketing_desc1}
            </p>
            <p className="text-lg text-slate-600 dark:text-slate-300 mb-8 leading-relaxed">
              {info.marketing_desc2}
            </p>
            <div className="flex flex-wrap gap-3">
              {parseItems(info.marketing_tags).map((tag, i) => (
      ...
      <section className="w-full py-24 px-6 md:px-12 max-w-[1200px] mx-auto">
        <motion.div 
          ...
          className="text-center mb-16"
        >
          <h2 className="text-3xl md:text-4xl font-bold font-montserrat mb-4 text-brand-navy dark:text-white">{info.qualif_title}</h2>
          <p className="text-slate-500 dark:text-slate-400 max-w-2xl mx-auto">{info.qualif_subtitle}</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          >
            ...
            <h3 className="text-2xl font-bold mb-4">{info.qualif_card1_title}</h3>
            <p className="text-slate-600 dark:text-slate-400 mb-6 text-sm">{info.qualif_card1_desc}</p>
            <ul className="space-y-3">
              {parseItems(info.qualif_card1_items).map((item, i) => (
          ...
          >
            ...
            <h3 className="text-2xl font-bold mb-4 relative z-10">{info.qualif_card2_title}</h3>
            <p className="text-slate-300 mb-6 text-sm relative z-10">{info.qualif_card2_desc}</p>
            <ul className="space-y-3 relative z-10">
              {parseItems(info.qualif_card2_items).map((item, i) => (
          ...
          >
            ...
            <h3 className="text-2xl font-bold mb-4">{info.qualif_card3_title}</h3>
            <p className="text-slate-600 dark:text-slate-400 mb-4 text-sm leading-relaxed">
              {info.qualif_card3_desc}
            </p>
            <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">
              {info.qualif_card3_desc2}
            </p>
          </motion.div>
        </div>
      </section>
      ...
      <section className="w-full py-24 px-6 md:px-12 bg-white/50 dark:bg-slate-900/30 backdrop-blur-md border-y border-slate-200 dark:border-slate-800">
        <div className="max-w-[1440px] mx-auto text-center">
          ...
          >
            ...
            <h2 className="text-3xl md:text-4xl font-bold font-montserrat mb-4 text-brand-navy dark:text-white">{info.crm_title}</h2>
            <p className="text-slate-500 dark:text-slate-400 max-w-3xl mx-auto text-sm md:text-base leading-relaxed">
              {info.crm_desc}
            </p>
          </motion.div>
          ...
          >
            <motion.div variants={fadeUp} className="w-full lg:w-1/2 rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700 relative group">
              ...
              <img src={info.crm_image1} alt="CRM preview 1" className="w-full h-auto object-cover transform group-hover:scale-105 transition-transform duration-700" />
            </motion.div>
            <motion.div variants={fadeUp} className="w-full lg:w-1/2 rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-slate-700 relative group">
              <div className="absolute inset-0 bg-blue-500/10 opacity-0 group-hover:opacity-100 transition-opacity z-10" />
              <img src={info.crm_image2} alt="CRM preview 2" className="w-full h-auto object-cover transform group-hover:scale-105 transition-transform duration-700" />
            </motion.div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="w-full py-24 px-6 md:px-12 max-w-[1200px] mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          >
            ...
            <h3 className="text-3xl font-bold font-montserrat mb-8">{info.benefits_title_client} <span className="text-slate-400 text-lg font-normal block mt-1">{info.benefits_subtitle_client}</span></h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-4">
              {parseItems(info.benefits_client_items).map((ben, i) => (
          ...
          >
            ...
            <h3 className="text-3xl font-bold font-montserrat mb-8 relative z-10">{info.benefits_title_partner} <span className="text-slate-400 text-lg font-normal block mt-1">{info.benefits_subtitle_partner}</span></h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-4 relative z-10">
              {parseItems(info.benefits_partner_items).map((ben, i) => (
                <div key={i} className="flex items-start gap-3">
      ...
      <section className="w-full py-24 px-6 md:px-12 bg-gradient-to-b from-transparent to-slate-100 dark:to-slate-900">
        <div className="max-w-[1000px] mx-auto text-center">
          ...
          >
            ...
            <h2 className="text-sm font-bold tracking-widest uppercase text-slate-400 mb-6">{info.mission_title}</h2>
            ...
            <div className="flex flex-wrap justify-center gap-4 md:gap-8 mb-12">
              {parseItems(info.mission_bullets).map((phrase, i) => (
